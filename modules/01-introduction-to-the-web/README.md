# Module 1: Introduction to the Web

A teacher-facing bundle for how the web works and minimal semantic structure.

## Files in this module

- Module overview: [module-overview.md](./module-overview.md)
- Units:
  - [Unit 1.1 — What is the Web & How It Works](./unit-1.1-lesson-plan.md)
  - [Unit 1.2 — Basic Structure of a Web Page](./unit-1.2-lesson-plan.md)
- Assessment: [assessment-rubric-01.md](./assessment-rubric-01.md)

## Related DPK resources

- [DPK module folder](https://github.com/STEAM-C3T/digital-proficiency-kit/tree/main/modules/01-introduction-to-the-web)

## Related Learning Materials

- [Learning Materials Module 01](https://github.com/STEAM-C3T/dpk-learning-materials/tree/main/modules/01-introduction-to-the-web) — Classroom-ready slides, tutorials, and workbooks
